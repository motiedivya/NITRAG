...
class ChunkingEvaluationManager:
    ...
    It intentionally reads raw chunks from rag_store/<doc_id>/chunks so the
    report measures only chunking behavior, before metadata enrichment,
    indexing, retrieval, reranking, or generation can affect the result.
    """
    ...
    def _page_coverage_rows(self, strategy: str, merged_intervals: List[Tuple[int, int]]) -> List[Dict[str, Any]]:
        ...
        for page in self.pages_df.itertuples():
            ...
            for start, end in merged_intervals:
                covered += max(0, min(end, page_end) - max(start, page_start))
            rows.append({
                "strategy": strategy,
        ...
        for strategy in strategies:
            ...
            if df.empty:
                metric_rows.append({
                    ...
                    "large_chunks_gt_1024": 0,
                    "oversized_chunks_gt_1500": 0,
            ...
            lengths = pd.to_numeric(df.get("token_length", df["end_index"] - df["start_index"]), errors="coerce").fillna(0)
            pages_per_chunk = pd.to_numeric(df.get("page_end", pd.Series(index=df.index)), errors="coerce") - pd.to_numeric(df.get("page_start", pd.Series(index=df.index)), errors="coerce") + 1
            pages_per_chunk = pages_per_chunk.fillna(0)
            ...
            metric_rows.append({
                ...
                "large_chunks_gt_1024": int((lengths > 1024).sum()),
                "oversized_chunks_gt_1500": int((lengths > 1500).sum()),
    ...
    def plot_all(
        ...
        if not lengths_df.empty:
            ...
            plt.title("Token length distribution by strategy")
            plt.tight_layout()
            plt.savefig(path, dpi=170)
            plt.close()
            paths.append(path)
            ...
            for ax, strategy in zip(axes, strategies):
                vals = lengths_df.loc[lengths_df["strategy"] == strategy, "token_length"].dropna()
        ...
        plt.barh(y, d["overlap_pct_of_chunk_tokens"], left=d["coverage_pct"], label="overlap % of chunk tokens", color="#f59e0b")
        missing_pct = 100 - d["coverage_pct"]
        plt.barh(y, missing_pct, left=d["coverage_pct"] + d["overlap_pct_of_chunk_tokens"], label="missing %", color="#ef4444")
        ...
        existing_boundary_cols = [c for c in boundary_cols if c in metrics_df.columns]
        if existing_boundary_cols:
            heat = metrics_df.set_index("strategy")[existing_boundary_cols].fillna(0)
            path = self.plots_dir / "05_boundary_alignment_heatmap.png"
            plt.figure(figsize=(max(11, len(existing_boundary_cols) * 1.25), max(4.5, len(heat) * 0.45)))
            ...
            plt.title("Boundary alignment and boundary cuts")
            for i in range(heat.shape[0]):
                for j in range(heat.shape[1]):
                    plt.text(j, i, f"{heat.values[i, j]:.0f}", ha="center", va="center", fontsize=8, color="white" if heat.values[i, j] < 55 else "black")
            plt.tight_layout()
            plt.savefig(path, dpi=170)
            plt.close()
            paths.append(path)
    ...
    def generate_report(self, strategies: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        (self.report_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
        return summary
